...
func TestEvaluateContractStrictness(t *testing.T) {
	...
	if len(violations) != 2 {
		t.Fatalf("lenient expected 2 violations, got %d", len(violations))
	}

	violations = Evaluate(c, req, 20, EnforcementModerate)
	if len(violations) != 3 {
		t.Fatalf("moderate expected 3 violations, got %d", len(violations))
	}

	violations = Evaluate(c, req, 20, EnforcementStrict)
	if len(violations) != 4 {
		t.Fatalf("strict expected 4 violations, got %d", len(violations))
	}
}
